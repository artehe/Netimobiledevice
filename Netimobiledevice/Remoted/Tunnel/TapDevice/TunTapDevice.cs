...
public class TunTapDevice : IDisposable {
    ...
    public void SetAddress(string value) {
        ...
        if (!string.IsNullOrEmpty(error)) {
            throw new TunnelException($"Failed to set IPv6 address. Error: {error}");
        }
    }
    ...
    public MIB_IPINTERFACE_ROW IpInterfaceEntry {
        get {
            ...
            if (result != 0) {
                throw new TunnelException($"Failed to get IP interface entry, error code: {result}");
            }
            return row;
        }
        set {
            ...
            if (result != 0) {
                throw new TunnelException($"Failed to set adapter MTU, error code: {result}");
            }
        }
    }
    ...
    public ulong Luid {
        get {
            ...
            Iphlpapi.InitializeIpInterfaceEntry(out _);
            return luid;
        }
    }
