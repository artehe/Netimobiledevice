...
internal static class TestFileHelper
{
    internal static Stream GetTestFileStream(string relativeFilePath)
    {
        ...
        if (string.IsNullOrEmpty(fullName)) {
            throw new FileLoadException($"Unable to find resource for path \"{relativeFilePath}\". Resource with name ending on \"{relativeName}\" was not found in assembly.");
        }

        Stream? stream = assembly.GetManifestResourceStream(fullName);
        return stream ?? throw new FileLoadException($"Unable to find resource for path \"{relativeFilePath}\". Resource named \"{fullName}\" was not found in assembly.");
    }
}
